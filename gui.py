...
class ImageProcessingApp:
    ...
    def _create_widgets(self):
        ...
        self.basic_button = ctk.CTkButton(self.app, text="Basic", command=lambda: self.select_tab(self.basic_tab))
        self.basic_button.grid(row=2, column=0, pady=10, padx=5)
        self.advanced_button = ctk.CTkButton(self.app, text="Advanced", command=lambda: self.select_tab(self.advanced_tab))
        self.advanced_button.grid(row=2, column=1, pady=10, padx=5)
        self.faces_button = ctk.CTkButton(self.app, text="Faces", command=lambda: self.select_tab(self.faces_tab))
        self.faces_button.grid(row=2, column=2, pady=10, padx=5)
        ...
        for tab in self.all_tabs:
            tab.grid(row=3, column=0, columnspan=3, sticky="nsew")
    ...
    def select_tab(self, selected_tab: ctk.CTkFrame):
        selected_tab.grid(row=3, column=0, columnspan=3, sticky="nsew")
        for tab in self.all_tabs:
    ...
    def _create_basic_widgets(self):
        ...
        self.rescale_checkbox = ctk.CTkCheckBox(
            self.basic_tab, text="Rescale", variable=self.should_rescale, command=lambda: self._toggle_frame(self.should_rescale, self.scale_frame)
        )
        ...
        self.resize_checkbox = ctk.CTkCheckBox(
            self.basic_tab, text="Resize", variable=self.should_resize, command=lambda: self._toggle_frame(self.should_resize, self.size_frame)
        )
        ...
        self.process_button = ctk.CTkButton(
            self.basic_tab, text="Process", command=self._process, width=self.BUTTON_WIDTH
        )
